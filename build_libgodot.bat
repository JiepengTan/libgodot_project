...
call scons platform=windows arch=x86_64 msvc=yes optimize=size
start /wait bin\godot.windows.editor.x86_64.exe --dump-extension-api
copy /y extension_api.json ..\build\extension_api.json

REM Build libgodot
call scons platform=windows arch=x86_64 msvc=yes library_type=shared_library optimize=size target=template_release
